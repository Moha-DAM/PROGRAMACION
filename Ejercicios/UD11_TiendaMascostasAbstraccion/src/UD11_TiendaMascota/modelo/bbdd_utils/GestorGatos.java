...
public class GestorGatos implements GestorInterfaz<Gato>{
    ...
    private ArrayList<Gato> buscarTodosGatos() {
	...
	@Override
	public ArrayList<Gato> obtenerTodos() {
		 ArrayList<Gato> gatos = buscarTodosGatos() ;
		return gatos;
	}

	@Override
	public Gato obtenerPorId(int id) {
		 ArrayList<Gato> gatos = obtenerTodos() ;
		 Gato gato = null;
		 for(int i = 0 ; i<gatos.size() ; i++ ) {
			 if (gatos.get(i).getIdCollar() == id) {
				 gato = gatos.get(i) ;
				 break;
			 }
		 }
